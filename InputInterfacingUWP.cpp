...
void Sample::DrawWheel(XMFLOAT2 startPosition)
{
    ...
    swprintf_s(wheelString, L"11111");
...
void Sample::UpdateWheel()
{
    ...
    {
        auto x=m_currentWheel->WheelMotor->SupportedAxes;
        ...
        m_effectcond = ref new ForceFeedback::ConditionForceEffect(ForceFeedback::ConditionForceEffectKind::Friction);
        m_effectcond->SetParameters(vector, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f);

        m_effectRamp = ref new ForceFeedback::RampForceEffect();
        Numerics::float3 vectore;
        vectore.x = 0.5;
        vectore.y = 0.f;
        vectore.z = 0.f;
        m_effectRamp->SetParameters(vector, vectore, time);

        IAsyncOperation<ForceFeedback::ForceFeedbackLoadEffectResult>^ request = m_currentWheel->WheelMotor->LoadEffectAsync(m_effectcond);
...
void Sample::Initialize(IUnknown* window, int width, int height, DXGI_MODE_ROTATION rotation)
{
    ...
    //Create an effect
  //  m_effect = ref new ForceFeedback::ConditionForceEffect(ForceFeedback::ConditionForceEffectKind());
   // TimeSpan time;
   // time.Duration = 10000;
   // Numerics::float3 vector;
   // vector.x = 1.f;
